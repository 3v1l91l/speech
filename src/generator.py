...
def batch_generator(should_augment, X, y, y_label, silences, unknowns, batch_size=16):
    while True:
        ...
        # known_flip_unknown_ix = np.random.choice(y_label[y_label in legal_labels_without_unknown_can_be_flipped].index, size=batch_size_unknown)
...
def batch_generator_paths(validate, X_paths, y, y_label, silences, unknowns, batch_size=16):
    while True:
        ...
        batch_size_unknown_flip_known = math.ceil(0.1 * batch_size)
        batch_size_silence = math.ceil(0.1 * batch_size)
        ...
        specgrams.extend(get_specgrams_augment_unknown(X[:len(all_unknown_ix)], len(unknown_ix) + np.array(range(len(unknown_flip_known_ix))), silences, unknowns))
        res_labels.extend(y[all_unknown_ix])

        specgrams.extend(get_specgrams_augment_silence(X[len(all_unknown_ix):len(all_unknown_ix) + len(silence_ix)], silences, unknowns))
        res_labels.extend(y[silence_ix])

        specgrams.extend(get_specgrams_augment_known(X[len(all_unknown_ix)+len(silence_ix):], silences, unknowns))
        res_labels.extend(y[known_ix])
