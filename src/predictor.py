...
all_folders = next(os.walk('../input/train/audio/'))[1]
all_labels = [x for x in all_folders if x != '_background_noise_']
cor_pred = []
all_num = []
labelbinarizer = LabelBinarizer()
labelbinarizer.fit(trained_labels)
for testing_label in all_labels:
    actual_label = testing_label
    if testing_label not in train_words:
        actual_label = 'unknown'
    test = prepare_data(get_path_label_df('c:\\code\\speech\\input\\train\\audio\\' + testing_label + '\\', '*.wav'))
    ...
    cor = np.sum(np.array(precicted_labels) == np.array([actual_label]))
    cor_pred.append(cor)
    all_num.append(len(np.array(precicted_labels)))
    print('Pct detected: {}'.format((100 * cor) / len(precicted_labels)))
    print('Pct detected as unknown: {}'.format((100 * np.sum(np.array(precicted_labels) == 'unknown')) / len(precicted_labels)))
