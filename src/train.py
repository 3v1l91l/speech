...
def validate(path, models):
    ...
    ix = np.random.choice(range(len(y_true)), 2000)
...
def train_model_one():
    ...
    model.fit_generator(
        ...
        validation_data=valid_gen,
        validation_steps=len(y_valid) // BATCH_SIZE // 4,
        callbacks=get_callbacks(label_index, 'model4'),
        workers=24,
        use_multiprocessing=False,
        verbose=1
    )
...
def validate_one():
    ...
    model = load_model('model3.model', custom_objects={'custom_accuracy_in': custom_accuracy(label_index),
                                                         'custom_loss_in': custom_loss(label_index)})
